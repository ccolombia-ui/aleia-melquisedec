...
def test_dry_run_minimal():
    r = subprocess.run(
        [sys.executable, str(SCRIPT), "--dry-run", "--minimal"], capture_output=True, text=True
    )
...
def test_requires_files_in_non_interactive_commit():
    ...
    r = subprocess.run(
        [sys.executable, str(SCRIPT), "--non-interactive", "--config", ".gitpush-temp.yml"],
        capture_output=True,
        text=True,
    )
    ...
    assert r.returncode != 0
    assert (
        "no commit message provided" in r.stdout
        or "no commit message provided" in r.stderr
        or "refusing" in r.stdout
    )

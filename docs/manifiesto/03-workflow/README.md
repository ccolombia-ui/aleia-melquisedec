# PARTE III: WORKFLOW Y GOBERNANZA

> **Carpeta**: `03-workflow/`
> **Versión**: 4.0.0
> **Última actualización**: 2026-01-08

---

## 📚 Contenido de esta Carpeta

Flujo de trabajo, gobernanza y gestión del conocimiento:

### Documentos Principales

1. **[01-kanban-estados.md](01-kanban-estados.md)** - Estados del issue (0.0 → 1.0)
2. **[02-trazabilidad.md](02-trazabilidad.md)** - Cadena derives_from → informs
3. **[03-versionamiento.md](03-versionamiento.md)** - Semantic versioning y alerts
4. **[04-mcps-recomendados.md](04-mcps-recomendados.md)** - MCPs por rostro
5. **[05-autopoiesis.md](05-autopoiesis.md)** - Sistema de autopoiesis y captura

---

## 🧭 Navegación

- **← Anterior**: [02. Arquitectura](../02-arquitectura/README.md)
- **→ Siguiente**: [04. Implementación](../04-implementacion/README.md)
- **↑ Índice**: [README Principal](../README.md)

---

**Última actualización**: 2026-01-08
